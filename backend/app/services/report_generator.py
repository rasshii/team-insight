...
class ReportGenerator:
    ...
    def generate_personal_report(self, user: User, db: Session, report_type: str = "weekly") -> Dict[str, Any]:
        """
        個人レポートを生成
        ...
        stats = (
            ...
            .filter(Task.assignee_id == user.id, Task.created_at >= start_date)
            .first()
        )
        ...
        deadline_stats = (
            ...
            .filter(Task.assignee_id == user.id, Task.due_date.isnot(None), Task.created_at >= start_date)
            .first()
        )

        deadline_adherence_rate = 0
        if deadline_stats.total_with_deadline > 0:
            deadline_adherence_rate = deadline_stats.completed_on_time / deadline_stats.total_with_deadline * 100
    ...
    def generate_project_report(self, project: Project, db: Session, report_type: str = "weekly") -> Dict[str, Any]:
        """
        プロジェクトレポートを生成
        ...
        stats = (
            ...
            .filter(Task.project_id == project.id, Task.created_at >= start_date)
            .first()
        )
        ...
        top_performers_data = (
            db.query(
                ...
                func.avg(func.extract("epoch", Task.completed_date - Task.created_at) / 86400).label("avg_completion_time"),
            )
            .join(Task, Task.assignee_id == User.id)
            .filter(Task.project_id == project.id, Task.status == TaskStatus.CLOSED, Task.completed_date >= start_date)
        ...
        top_performers = [
            {"name": name, "completed_tasks": completed, "avg_completion_time": round(avg_time, 1) if avg_time else 0}
    ...
    def generate_team_report(self, db: Session, report_type: str = "weekly") -> Dict[str, Any]:
        """
        チーム全体のレポートを生成
        ...
        report_data = {
            ...
            "overall_completion_rate": round(
                (overall_stats.completed / overall_stats.total * 100) if overall_stats.total > 0 else 0, 1
            ),
    ...
    def _get_report_type_label(self, report_type: str) -> str:
        """レポートタイプのラベルを取得"""
        labels = {"daily": "日次", "weekly": "週次", "monthly": "月次"}
