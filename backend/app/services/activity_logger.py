...
class ActivityLogger:
    ...
    def log_login(db: Session, user: User, request: Optional[Request] = None):
        ...
        return ActivityLogger.log_activity(
            db=db, user=user, action="login", resource_type="user", resource_id=user.id, request=request
        )
    ...
    def log_logout(db: Session, user: User, request: Optional[Request] = None):
        """ログアウトアクティビティを記録"""
        return ActivityLogger.log_activity(
            db=db, user=user, action="logout", resource_type="user", resource_id=user.id, request=request
        )

    @staticmethod
    def log_settings_update(db: Session, user: User, updated_fields: Dict[str, Any], request: Optional[Request] = None):
        """設定更新アクティビティを記録"""
        return ActivityLogger.log_activity(
            db=db,
            user=user,
