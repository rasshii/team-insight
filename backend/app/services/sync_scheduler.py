...
class SyncSchedulerService:
    ...
    def add_custom_sync_job(self, job_id: str, sync_type: str, schedule: Dict[str, Any], user_id: Optional[int] = None):
        """
        カスタム同期ジョブを追加
    ...
    def _get_valid_token(self, user: User, db: Session) -> Optional[OAuthToken]:
        ...
        if token.is_expired() or token_refresh_service._should_refresh_token(token):
            try:
                refreshed_token = token_refresh_service.refresh_token_sync(token, db, settings.BACKLOG_SPACE_KEY)
                if refreshed_token:
    ...
    async def _sync_all_users(self):
        ...
        try:
            ...
            result = await sync_service.import_users_from_backlog(
                admin_user, token.access_token, db, mode="active_only", assign_default_role=True
            )
    ...
    async def _sync_all_projects(self):
        ...
        try:
            ...
            # プロジェクト同期を実行
            result = await sync_service.sync_all_projects(admin_user, token.access_token, db)
    ...
    async def _sync_active_project_tasks(self):
        ...
        try:
            ...
            active_projects = db.query(Project).filter(Project.status == "active").limit(10).all()  # 負荷を考慮して上限を設定

            logger.info(f"Starting scheduled task sync for {len(active_projects)} active projects")

            for project in active_projects:
                try:
                    result = await sync_service.sync_project_tasks(project, token.access_token, db, admin_user)
