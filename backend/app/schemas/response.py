...
class SuccessResponse(GenericModel, Generic[DataT]):
    ...
    timestamp: str = Field(default_factory=lambda: datetime.utcnow().isoformat(), description="レスポンス生成時刻")
        ...
        schema_extra = {
            "example": {
                "error": {
                    ...
                    "details": [{"field": "email", "reason": "有効なメールアドレスではありません", "value": "invalid-email"}],
                },
                "status_code": 422,
            }
        }
...
class BulkOperationResult(BaseModel):
    ...
    errors: List[ErrorDetail] = Field(default_factory=list, description="エラー詳細リスト")
