...
async def get_connection_status(
    ...
    """
    # OAuthトークンを取得
    oauth_token = db.query(OAuthToken).filter(OAuthToken.user_id == current_user.id, OAuthToken.provider == "backlog").first()
...
async def test_connection(
    ...
    if not oauth_token:
        raise AppException(
            error_code=ErrorCode.NOT_FOUND, detail="Backlog連携が設定されていません", status_code=status.HTTP_404_NOT_FOUND
        )
    ...
    try:
        ...
        return formatter.success(
            data={
                ...
                "user_info": {"id": user_info.get("id"), "name": user_info.get("name"), "email": user_info.get("mailAddress")},
            }
        )
        ...
        return formatter.success(data={"success": False, "message": error_message, "user_info": None})
...
async def disconnect_backlog(
    ...
    """
    # OAuthトークンを削除
    deleted = db.query(OAuthToken).filter(OAuthToken.user_id == current_user.id, OAuthToken.provider == "backlog").delete()

    if deleted == 0:
        raise AppException(
            error_code=ErrorCode.NOT_FOUND, detail="Backlog連携が見つかりません", status_code=status.HTTP_404_NOT_FOUND
        )

    db.commit()

    return formatter.success(data={"success": True, "message": "Backlog連携を解除しました"})
...
async def get_project_statuses(
    ...
    if not project:
        raise AppException(
            error_code=ErrorCode.NOT_FOUND, detail="プロジェクトが見つかりません", status_code=status.HTTP_404_NOT_FOUND
        )
    ...
    if cached_data:
        try:
            ...
            return formatter.success(
                data={"statuses": statuses, "cached": True}, message="ステータス一覧を取得しました（キャッシュ）"
            )
    ...
    try:
        ...
        statuses = await backlog_client.get_issue_statuses(project_id=backlog_project_id, access_token=access_token)
        ...
        return formatter.success(data={"statuses": statuses, "cached": False}, message="ステータス一覧を取得しました")
...
async def get_user_project_statuses(
    ...
    if not projects:
        return formatter.success(data={"projects": [], "all_statuses": []}, message="参加しているプロジェクトがありません")
    ...
    for project in projects:
        try:
            # Backlog APIからステータス一覧を取得
            statuses = await backlog_client.get_issue_statuses(project_id=project.backlog_id, access_token=access_token)
            ...
            for status_item in statuses:
                all_statuses_map[status_item["id"]] = status_item
    ...
    all_statuses = sorted(all_statuses_map.values(), key=lambda x: x.get("displayOrder", 999))

    return formatter.success(
        data={"projects": project_statuses, "all_statuses": all_statuses, "cached": False},
