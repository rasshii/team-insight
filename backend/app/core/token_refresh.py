...
class TokenRefreshService:
    ...
    async def get_valid_token(user: User, db: Session) -> Optional[str]:
        ...
        expires_at = oauth_token.expires_at.replace(tzinfo=timezone.utc) if oauth_token.expires_at.tzinfo is None else oauth_token.expires_at
    ...
    async def ensure_valid_token(user: User, db: Session) -> str:
        ...
        if not token:
            raise AuthenticationException(
                "Backlogアクセストークンが見つかりません。Backlog連携を行ってください。"
            )
